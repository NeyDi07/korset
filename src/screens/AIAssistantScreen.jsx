...
const CHIPS = {
  ru: ['Есть ли халал продукты?', 'Что без лактозы?', 'Где найти орехи?', 'Что для рататуя?'],
  kz: ['Халал тауарлар бар ма?', 'Лактозасыз не бар?', 'Жаңғақ қайда?', 'Рататуйға не керек?'],
}
...
export default function AIAssistantScreen() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const system = lang === 'kz'
        ? 'Сен — Қазақстан супермаркетіндегі Körset AI көмекшісісің. Тауар табуға, рецепт ұсынуға және құрамын түсіндіруге көмектес. Қысқа, түсінікті қазақша жауап бер.'
        : 'Ты — Körset AI, помощник покупателя в супермаркете Казахстана. Помогаешь найти товары, советуешь рецепты, отвечаешь про состав и аллергены. Кратко, по-русски, как дружелюбный консультант.'
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
        body: JSON.stringify({ model: 'llama-3.1-8b-instant', max_tokens: 300, temperature: 0.7, messages: [{ role: 'system', content: system }, ...newMessages] }),
      })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
      const data = await res.json()
      const reply = data.choices?.[0]?.message?.content?.trim()
      setMessages(prev => [...prev, { role: 'assistant', content: reply || (lang === 'kz' ? 'Тағы бір рет көріңіз.' : 'Попробуй ещё раз.') }])
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', content: lang === 'kz' ? 'Бірдеңе дұрыс болмады, тағы бір рет көріңіз.' : 'Что-то пошло не так, попробуй ещё раз.' }])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--bg)' }}>
      <div style={{ padding: '16px 20px 14px', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#34D399', fontWeight: 500, marginTop: 1 }}>{lang === 'kz' ? 'Дүкендегі көмекші' : 'Помощник в магазине'}</div>
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 140px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {messages.length === 0 && <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}><KorsetAvatar size={34}/><div style={{ background: '#151525', border: '1px solid rgba(255,255,255,0.08)', padding: '13px 16px', borderRadius: '4px 18px 18px 18px', maxWidth: '85%', fontSize: 15, lineHeight: 1.65, color: 'rgba(255,255,255,0.85)' }}>{lang === 'kz' ? 'Сәлем! Кез келген тауар, рецепт немесе дүкеннен не табу туралы сұраңыз 🛒' : 'Привет! Спроси меня про любой товар, рецепт или что найти в магазине 🛒'}</div></div>}
