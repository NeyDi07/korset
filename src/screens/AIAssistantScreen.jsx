    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--bg)' }}>
      <div style={{ padding: '16px 20px 14px', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#34D399', fontWeight: 500, marginTop: 1 }}>{t.ai.generalSubtitle}</div>
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 140px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {messages.length === 0 && <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}><KorsetAvatar size={34}/><div style={{ background: '#151525', border: '1px solid rgba(255,255,255,0.08)', padding: '13px 16px', borderRadius: '4px 18px 18px 18px', maxWidth: '85%', fontSize: 15, lineHeight: 1.65, color: 'rgba(255,255,255,0.85)' }}>{t.ai.welcomeGeneral}</div></div>}
      ...
      <div style={{ position: 'fixed', left: 0, right: 0, bottom: '86px', zIndex: 90, padding: '8px 16px 16px', background: '#0C0C18', borderTop: '1px solid rgba(255,255,255,0.06)', paddingBottom: 'calc(16px + env(safe-area-inset-bottom))' }}>
        {messages.length === 0 && <div style={{ display: 'flex', gap: 8, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 10 }}>{t.ai.generalChips.map(chip => <button key={chip} onClick={() => sendMessage(chip)} style={{ flexShrink: 0, padding: '7px 14px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>{chip}</button>)}</div>}
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(input) } }} placeholder={t.ai.inputGeneral} disabled={loading} style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, padding: '13px 18px', fontSize: 15, color: '#fff', fontFamily: 'var(--font-body)', outline: 'none' }} />
