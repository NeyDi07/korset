...
const OPTIONS = [
  { id: 'halal', emoji: '☪️', apply: p => ({ ...p, halalOnly: true }) },
  { id: 'allergy_milk', emoji: '🥛', apply: p => ({ ...p, allergens: [...(p.allergens||[]).filter(a=>a!=='milk'), 'milk'] }) },
  { id: 'allergy_gluten', emoji: '🌾', apply: p => ({ ...p, allergens: [...(p.allergens||[]).filter(a=>a!=='gluten'), 'gluten'], dietGoals: [...(p.dietGoals||[]).filter(g=>g!=='gluten_free'), 'gluten_free'] }) },
  { id: 'allergy_nuts', emoji: '🥜', apply: p => ({ ...p, allergens: [...(p.allergens||[]).filter(a=>a!=='nuts'&&a!=='peanuts'), 'nuts', 'peanuts'] }) },
  { id: 'sugar_free', emoji: '🚫', apply: p => ({ ...p, sugarFree: true, dietGoals: [...(p.dietGoals||[]).filter(g=>g!=='sugar_free'), 'sugar_free'] }) },
  { id: 'vegan', emoji: '🌱', apply: p => ({ ...p, dietGoals: [...(p.dietGoals||[]).filter(g=>g!=='vegan'), 'vegan'] }) },
]
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'var(--bg, #07070F)', display: 'flex', flexDirection: 'column', opacity: leaving ? 0 : 1, transition: 'opacity 0.3s ease', overflowY: 'auto' }}>
      <div style={{ padding: '44px 24px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['ru','kz'].map(code => (
              <button key={code} onClick={() => setLang(code)} style={{ padding: '8px 12px', borderRadius: 12, border: code===lang ? '1px solid rgba(124,58,237,0.8)' : '1px solid rgba(255,255,255,0.08)', background: code===lang ? 'rgba(124,58,237,0.18)' : 'rgba(255,255,255,0.04)', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
        ...
        <div style={{ fontSize: 12, color: 'rgba(180,180,210,0.72)', marginBottom: 12 }}>{t('onboarding.chooseLang')}</div>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 30, fontWeight: 900, lineHeight: 1.15, color: '#fff', marginBottom: 10, letterSpacing: '-0.5px' }}>
          ...
          <span style={{ background: 'linear-gradient(90deg, #A78BFA, #7C3AED)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{t('onboarding.title2')}</span>
        </h1>
        <p style={{ fontSize: 15, color: 'rgba(180,180,210,0.8)', lineHeight: 1.55, marginBottom: 22 }}>{t('onboarding.subtitle')}</p>
        <div style={{ fontSize: 12, color: 'rgba(180,180,210,0.72)', marginBottom: 12 }}>{t('onboarding.choosePrefs')}</div>
      </div>
      ...
      <div style={{ padding: '0 24px', flex: 1 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {OPTIONS.map(opt => {
            ...
            const [label, sub] = t(`onboarding.opts.${opt.id}`)
            return (
              <button key={opt.id} onClick={() => toggle(opt.id)} style={{ padding: '14px 14px 13px', borderRadius: 16, border: on ? '1.5px solid rgba(124,58,237,0.7)' : '1.5px solid rgba(255,255,255,0.08)', background: on ? 'linear-gradient(135deg, rgba(124,58,237,0.22), rgba(109,40,217,0.12))' : 'rgba(255,255,255,0.04)', cursor: 'pointer', textAlign: 'left', transition: 'all 0.18s ease', boxShadow: on ? '0 0 20px rgba(124,58,237,0.2)' : 'none', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: on ? '#E9D5FF' : 'rgba(220,220,240,0.9)', marginBottom: 2, fontFamily: 'var(--font-display)' }}>{label}</div>
                <div style={{ fontSize: 11, color: on ? 'rgba(196,181,253,0.8)' : 'rgba(140,140,170,0.7)', lineHeight: 1.4 }}>{sub}</div>
              </button>
            )
          })}
        </div>
      ...
      <div style={{ padding: '20px 24px 44px', flexShrink: 0 }}>
        <button onClick={finish} style={{ width: '100%', padding: '16px', borderRadius: 16, border: 'none', cursor: 'pointer', background: selected.size > 0 ? 'linear-gradient(135deg, #7C3AED, #6D28D9)' : 'rgba(124,58,237,0.35)', color: '#fff', fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 700, boxShadow: selected.size > 0 ? '0 4px 24px rgba(124,58,237,0.4)' : 'none', transition: 'all 0.2s ease', marginBottom: 10 }}>
          {selected.size > 0 ? t('onboarding.start') : t('onboarding.skipSetup')}
        </button>
        {selected.size > 0 && <button onClick={skip} style={{ width: '100%', padding: '12px', background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(140,140,170,0.7)', fontSize: 13, fontFamily: 'var(--font-body)' }}>{t('onboarding.skipSmall')}</button>}
      </div>
    </div>
  )
}
