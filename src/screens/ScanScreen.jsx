...
function BarcodeScanner({ onDetected, onClose }) {
  ...
  const closingRef = useRef(false) // guard against race condition
  const ID = 'korset-qr-reader'

  const handleClose = async () => {
    if (closingRef.current) return
    closingRef.current = true
    // Stop scanner first, THEN call onClose
    try {
    ...
    async function start() {
      try {
        ...
        await scanner.start(
          ...
          (text) => {
            // Don't process if we're already closing
        ...
        if (/permission|not allowed|denied/i.test(msg)) {
          setError('Доступ к камере отклонён.\nОткройте настройки браузера → разрешите камеру → обновите страницу.')
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#000' }}>
        ...
        {ready && !error && (
          <div style={{
            ...
            {/* frame */}
            <div style={{ position: 'relative', zIndex: 2, width: 250, height: 250, boxShadow: '0 0 0 9999px rgba(0,0,0,0.45)', borderRadius: 12 }}>
              {[
        ...
        {error && (
          <div style={{
            ...
            <button
              onClick={onClose}
              style={{ padding: '13px 36px', borderRadius: 14, background: '#7C3AED', border: 'none', color: '#fff', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}
            >
              ← Назад
            </button>
          </div>
        ...
        {/* Close button — always visible when no error */}
        {!error && (
          <button
            onClick={handleClose}
            style={{
      ...
      {!error && (
        <div style={{
          ...
          <p style={{ color: '#58587A', fontSize: 12, marginTop: 6 }}>
            Сканируйте QR-коды Körset с ценников
          </p>
        </div>
      )}
...
export default function ScanScreen() {
  ...
  const handleDetected = (text) => {
    ...
    if (product) {
      ...
      // fallback по EAN на случай если кто-то сканирует обычный штрихкод
      const byEan = products.find(p => p.ean === text)
      if (byEan) navigate(`/product/${byEan.id}`)
      // если совсем неизвестно — остаёмся на этой странице
    }
  }
        ...
        <div style={{ padding: '0 20px 16px' }}>
          ...
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <polyline points="6 15 6 20 18 20 18 15"/>
              <rect x="3" y="9" width="18" height="6"/>
            </svg>
            Распечатать QR-коды для ценников
          </button>
        </div>
              ...
              return (
                ...
                >
                  <div className="product-emoji" style={{
                    background: 'var(--primary-dim)', border: '1px solid rgba(139,92,246,0.15)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
