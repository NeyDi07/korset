...
/* ─── Error i18n mapping ─── */
const errMap = {
  'Invalid login credentials': { ru: 'Неверный email или пароль', kz: 'Қате email немесе құпиясөз' },
  'Email not confirmed': { ru: 'Email не подтверждён. Проверьте почту', kz: 'Email расталмаған. Поштаңызды тексеріңіз' },
  'User already registered': { ru: 'Этот email уже зарегистрирован', kz: 'Бұл email тіркелген' },
  'Password should be at least 6 characters': { ru: 'Пароль должен быть минимум 6 символов', kz: 'Құпиясөз кемінде 6 таңба болуы керек' },
...
export default function AuthScreen() {
  ...
  const canSubmit = () => {
    ...
    if (mode === 'register') return email && password && confirmPassword && pwErrors.length === 0 && !pwMismatch
  ...
  const inputStyle = (field) => ({
    ...
    borderRadius: 14, color: '#fff', fontSize: 14, fontFamily: fontAdvent,
  ...
  const titles = {
    login: { title: lang === 'kz' ? 'Қош келдіңіз' : 'С возвращением', sub: lang === 'kz' ? 'Аккаунтқа кіріңіз' : 'Войдите в аккаунт Körset' },
    register: { title: lang === 'kz' ? 'Тіркелу' : 'Создать аккаунт', sub: lang === 'kz' ? 'Körset-ке қосылыңыз' : 'Присоединяйтесь к Körset' },
    verify: { title: lang === 'kz' ? 'Кодты растау' : 'Подтверждение', sub: lang === 'kz' ? 'Поштаңызға жіберілген 6-таңбалы кодты енгізіңіз' : `Код отправлен на ${email}` },
    forgot: { title: lang === 'kz' ? 'Құпиясөзді қалпына келтіру' : 'Сброс пароля', sub: lang === 'kz' ? 'Email-ді енгізіңіз' : 'Введите email для восстановления' },
  }
    ...
    <>
      ...
      <div className="screen auth-screen" style={{
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -60, left: -60, width: 200, height: 200, borderRadius: '50%', background: 'rgba(124,58,237,0.12)', filter: 'blur(70px)', animation: 'floatA1 8s ease-in-out infinite' }} />
          <div style={{ position: 'absolute', top: '40%', right: -40, width: 160, height: 160, borderRadius: '50%', background: 'rgba(236,72,153,0.08)', filter: 'blur(60px)', animation: 'floatA2 10s ease-in-out infinite' }} />
          <div style={{ position: 'absolute', bottom: 60, left: 40, width: 120, height: 120, borderRadius: '50%', background: 'rgba(52,211,153,0.06)', filter: 'blur(50px)', animation: 'floatA1 12s ease-in-out infinite' }} />
        </div>
        ...
        <div style={{ padding: '0 22px 28px', position: 'relative', zIndex: 10, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
          <div style={{
            ...
            {error && (
              <div style={{
                ...
                fontFamily: fontAdvent, marginBottom: 18, textAlign: 'center',
            ...
            {success && (
              <div style={{
                ...
                color: '#34D399', padding: '12px 16px', borderRadius: 12, fontSize: 13,
                fontFamily: fontAdvent, marginBottom: 18, textAlign: 'center',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8
              }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                {success}
              </div>
            )}
                  ...
                  {otp.map((digit, i) => (
                    <input
                      ...
                      style={{
                        ...
                        background: 'rgba(255,255,255,0.04)', border: '1.5px solid rgba(124,58,237,0.2)',
                        borderRadius: 12, color: '#fff', outline: 'none', transition: 'border 0.2s'
                      }}
                    />
                  ))}
                </div>
                <button type="submit" disabled={loading || otp.join('').length < 6} style={{
                  background: '#7C3AED', color: '#fff', border: 'none', padding: 16, borderRadius: 14,
                  fontSize: 15, fontWeight: 600, fontFamily: fontAdvent, cursor: 'pointer',
                  opacity: (loading || otp.join('').length < 6) ? 0.5 : 1, transition: 'opacity 0.2s'
                }}>
            ...
            /* ── FORGOT MODE ── */
              ...
              <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <button type="button" onClick={() => switchMode('login')} style={{
                  ...
                  fontFamily: fontAdvent, cursor: 'pointer', marginTop: 4
                }}>
              ...
              <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {mode === 'register' && password.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '0 2px' }}>
                    {[
                      { ok: password.length >= 8, text: lang === 'kz' ? 'Кемінде 8 таңба' : 'Минимум 8 символов' },
                      { ok: /[0-9]/.test(password), text: lang === 'kz' ? 'Кемінде 1 сан' : 'Минимум 1 цифра' },
                      { ok: /[A-Za-z]/.test(password), text: lang === 'kz' ? 'Кемінде 1 әріп' : 'Минимум 1 буква' },
                    ].map((rule, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ width: 14, height: 14, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: rule.ok ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.05)' }}>
                          {rule.ok ? (
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#34D399" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                          ) : (
                            <div style={{ width: 4, height: 4, borderRadius: '50%', background: 'rgba(255,255,255,0.2)' }} />
                          )}
                        </div>
                        <span style={{ fontSize: 11, fontFamily: fontAdvent, color: rule.ok ? '#34D399' : 'rgba(255,255,255,0.3)' }}>{rule.text}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {mode === 'register' && (
                  <div style={{ position: 'relative' }}>
                    <input type={showConfirmPassword ? 'text' : 'password'} className="auth-input"
                      ...
                      style={{
                        ...inputStyle('cpw'),
                        borderColor: pwMismatch ? 'rgba(239,68,68,0.5)' : (focusedField === 'cpw' ? 'rgba(124,58,237,0.5)' : 'rgba(255,255,255,0.08)')
                      }} />
                    <EyeBtn show={showConfirmPassword} onToggle={() => setShowConfirmPassword(!showConfirmPassword)} />
                    {pwMismatch && (
                      <div style={{ fontSize: 11, color: '#F87171', marginTop: 4, fontFamily: fontAdvent, paddingLeft: 2 }}>
                ...
                {mode === 'login' && (
                  <div style={{ textAlign: 'right', marginTop: -4 }}>
                    <button type="button" onClick={() => switchMode('forgot')} style={{
                      background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)',
                      fontSize: 12, fontFamily: fontAdvent, cursor: 'pointer'
                    }}>
                ...
                <button type="submit" disabled={loading || !canSubmit()} style={{
                  background: '#7C3AED', color: '#fff', border: 'none', padding: 16, borderRadius: 14,
                  fontSize: 15, fontWeight: 600, fontFamily: fontAdvent, cursor: 'pointer',
                  opacity: (loading || !canSubmit()) ? 0.5 : 1, marginTop: 6, transition: 'opacity 0.2s'
                }}>
                  {loading ? '...' : (mode === 'login'
                    ? (lang === 'kz' ? 'Кіру' : 'Войти')
            ...
            {mode !== 'verify' && mode !== 'forgot' && (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '22px 0' }}>
                  ...
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', fontFamily: fontAdvent, textTransform: 'uppercase', letterSpacing: 1 }}>
                    {lang === 'kz' ? 'немесе' : 'или'}
                  </span>
                  <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.06)' }} />
                </div>
                ...
                <button onClick={handleGoogleAuth} style={{
                  ...
                  color: '#fff', padding: 14, borderRadius: 14, fontSize: 14, fontWeight: 500,
                  fontFamily: fontAdvent, cursor: 'pointer', transition: 'background 0.15s'
                }}>
                  ...
                  {mode === 'login'
                    ...
                    : (lang === 'kz' ? 'Google арқылы тіркелу' : 'Регистрация через Google')}
                </button>
              </>
            )}
          </div>
          ...
          {mode !== 'verify' && mode !== 'forgot' && (
            <div style={{ textAlign: 'center', marginTop: 28 }}>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', fontFamily: fontAdvent, margin: 0 }}>
                {mode === 'login'
              ...
              <button onClick={() => switchMode(mode === 'login' ? 'register' : 'login')} style={{
                background: 'none', border: 'none', color: '#A78BFA', fontSize: 14, fontWeight: 600,
                fontFamily: fontAdvent, cursor: 'pointer', marginTop: 6
              }}>
                {mode === 'login'
                  ? (lang === 'kz' ? 'Тіркелу' : 'Зарегистрироваться')
