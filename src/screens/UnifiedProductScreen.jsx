    ...
    >
      <div
        style={{
          ...
          alignItems: 'center',
          gap: 10,
          marginBottom: reasons.length ? 12 : 0,
        }}
      >
        <div
          style={{
        ...
        >
          ...
          <div style={{ color: 'var(--text)', fontWeight: 800, fontSize: 18 }}>
            ...
            {value == null ? (
              ...
              <span
                style={{ color: 'var(--text-dim)', fontWeight: 600, fontSize: 12, marginLeft: 4 }}
              >
                {unit}
              </span>
            )}
...
export default function UnifiedProductScreen({ mode = 'canonical' }) {
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      ...
      setError(null)
      try {
        let nextProduct = location.state?.product
          ? coerceProductEntity(location.state.product)
          : null
        if (!nextProduct) {
          const ref = mode === 'external' ? { ean } : { canonicalId: id }
  ...
  const manufacturerText = [product?.manufacturer?.name, product?.manufacturer?.country]
    .filter(Boolean)
    .join(' · ')
  ...
  const goToAlternatives = () => {
    if (!product) return
    navigate(`/product/${encodeURIComponent(product.canonicalId)}/alternatives`, {
      state: { product },
    })
  }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          >
            <HeartIcon
              filled={isFavorite}
              size={18}
              color={isFavorite ? '#F87171' : 'currentColor'}
            />
          </button>
        </div>
      </div>
