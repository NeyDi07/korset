...
import { ALLERGEN_NAMES } from '../constants/allergens.js'
...
export function checkProductFit(product, profile) {
  ...
  if (
    ...
    (dietTags.includes('contains_dairy') || (product.allergens || []).includes('milk'))
  )
  ...
  if (fits) {
    ...
    if (allergens.length > 0 && (product.allergens || []).length === 0)
