...
// ── 0. Demo store catalog — временно ведём Store One как полноценный тестовый магазин ──
...
export async function lookupProduct(ean, storeId = null) {
  // 0. Demo store catalog (Store One) — должен иметь приоритет в store context,
  // чтобы скан и каталог открывали одну и ту же карточку по EAN.
  ...
  // 3. Fallback: локальный демо-каталог реальных SKU
  const local = getGlobalDemoProducts().find(p => p.ean === ean) || localProducts.find(p => p.ean === ean)
  if (local) {
