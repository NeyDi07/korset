...
> **⚠️ ПРАВИЛО ЧТЕНИЯ:** Информация в этом файле расположена от старой к новой. Если данные противоречат друг другу — более нижний раздел имеет приоритет. Любой сеанс работы ИИ-ассистента или разработчика должен начинаться с анализа этого документа.
...
> ‼️ **ВАЖНО:** Файлы `products.json` и любые локальные моки **ПОЛНОСТЬЮ УДАЛЕНЫ** из архитектуры. ИИ-ассистенты не должны на них ссылаться.
...
| `src/screens/ScanScreen.jsx` | Экран сканера (⚠️ хрупкий, есть memory leak AudioContext) |
...
| `supabase_schema.sql` | Полная боевая схема БД |
| `supabase_push_notifications.sql` | SQL-схема для Push-уведомлений |
| `KORSET_DB_SCHEMA_FULL.sql` | Контекстная схема БД (только для чтения) |
...
> Roadmap согласован 2026-04-14. Текущий статус задач — в `TODO_V1.md`.
