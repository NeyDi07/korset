...
- [ ] **Offline Sync (Сканер без интернета):** Сохранение события `scan_event` (сканирования вне зоны 4G, в подвалах) локально и отправка в Supabase при возвращении сети. Критично для метрики «Упущенная выгода».
